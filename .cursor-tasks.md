...
[ ] Implement JavaScript to handle the change event on the pdfUpload input.
[ ] Send the selected file to a backend endpoint (e.g., /upload).
[ ] Implement a basic backend endpoint to receive the PDF file.
[ ] Store the uploaded PDF file temporarily on the server.
[ ] Send a success response (e.g., 200 OK) back to the frontend upon successful upload.
[ ] Display a success message to the user (e.g., "PDF uploaded successfully").
User Story 3: PDF Upload - Invalid File Type

[ ] In the frontend JavaScript, check the file.type property to ensure it is "application/pdf".
[ ] If the file type is not PDF, display an error message to the user (e.g., "Please upload a PDF file").
[ ] Ensure the backend also validates the file type.
[ ] If the backend detects an invalid file type, send an appropriate error response (e.g., 400 Bad Request).
