...
export function trackEvent(type: EventType, target: string, metadata?: Record<string, any>) {
  // Try to get the current session if we're in a browser environment
  let session: any = null;
  
  if (typeof window !== 'undefined') {
    try {
      // This is NOT a React hook, it's accessing a global value that Next Auth sets
      // Note: This will only work if NextAuth has already populated the session in the global state
      const globalSession = (window as any).__NEXTAUTH?.session;
      if (globalSession?.user) {
        session = globalSession;
      }
    } catch (e) {
      console.log("[Analytics] Could not access session:", e);
    }
  }
  
  // Include user email in metadata if available from session
  const enhancedMetadata = {
    ...metadata
  };
  
  if (session?.user?.email && !enhancedMetadata.email) {
    enhancedMetadata.email = session.user.email;
  }
  
  if (session?.user?.id && !enhancedMetadata.userId) {
    enhancedMetadata.userId = session.user.id;
  }
